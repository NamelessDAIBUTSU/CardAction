...
void AElectricThunder::BeginPlay()
{
	...
	if (ThunderEffect)
	{
		...
		GetWorldTimerManager().SetTimer(PlayEffectHandle, this, &AElectricThunder::OnPlayEffect, 1.5f, false);
	}

	// ダメージ処理を指定タイミングで行う
	FTimerHandle DamageTimerHandle;
	GetWorldTimerManager().SetTimer(DamageTimerHandle, this, &AElectricThunder::OnAttack, 2.f, false);
}
